...
<head>
    ...
    <title>Mangatech-Admin dashboard-Animes</title>
    ...
    <div class="w-full flex flex-col h-screen overflow-y-hidden">
        ...
        <div class="w-full overflow-x-hidden border-t flex flex-col">
            <main class="w-full flex-grow p-6">
                ...
                <div class="flex">
                    <h1 class="text-3xl text-black pb-6 flex-1">Animes</h1>
                    <a class="bg-green-500 text-gray-100 self-center py-2 px-2 font-bold "
                        href="{{ route('admin.animes.create') }}">
                        Create new anime
                    </a>
                </div>
                    ...
                    <div class="bg-white overflow-auto">
                        <table class="min-w-full bg-white">
                            ...
                            <tbody class="text-gray-700">
                                ...
                                <tr>
                                    <td class="w-1/3 text-left font-bold py-3 px-4">{{ $anime->id }}</td>
                                    <td class="w-1/3 text-left py-3 px-4">{{ $anime->slug }}</td>
                                    <td class="w-1/3 text-left py-3 px-4">{{ $anime->title }}</td>
                                    <td class="w-1/3 text-left py-3 px-4">{{ $anime->description }}</td>
                                    <td class="w-1/3 text-left py-3 px-4"><img src="{{ asset('images/' . $anime->image) }}"></td>
                                    <td class="w-1/3 text-left py-3 px-4">{{ $anime->author }}</td>
                                    <td class="w-1/3 text-left py-3 px-4">{{ $anime->trailer }}</td>
                                    <td class="w-1/3 text-left py-3 px-4">
                                        <ul>
                                            @foreach ($anime->tags as $tags)
                                                <li>{{ $tags->name }}</li>
                                            @endforeach
                                        </ul>
                                    </td>
                                    ...
                                    <td class="w-1/3 text-left py-3 px-4">
                                        ...
                                        href="{{ route('admin.animes.edit', $anime->id) }}">
                                            Edit
                                        </a>

                                        {{-- Delete button --}}
                                        <form action="{{ route('admin.animes.destroy', $anime->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
