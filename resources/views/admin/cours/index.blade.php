...
<head>
    ...
    <title>Greatapp-Admin dashboard-Cours</title>
    ...
    <div class="w-full flex flex-col h-screen overflow-y-hidden">
        ...
        <div class="w-full overflow-x-hidden border-t flex flex-col">
            <main class="w-full flex-grow p-6">
                ...
                <div class="flex">
                    <h1 class="text-3xl text-black pb-6 flex-1">Cours</h1>
                    ...
                    <div class="bg-white overflow-auto">
                        <table class="min-w-full bg-white">
                            ...
                            <tbody class="text-gray-700">
                                ...
                                <tr>
                                    <td class="w-1/3 text-left font-bold py-3 px-4">{{ $cour->id }}</td>
                                    <td class="w-1/3 text-left py-3 px-4">{{ $cour->slug }}</td>
                                    <td class="w-1/3 text-left py-3 px-4">{{ $cour->title }}</td>
                                    <td class="w-1/3 text-left py-3 px-4">{{ $cour->description }}</td>
                                    <td class="w-1/3 text-left py-3 px-4"><img src="{{ asset('images/' . $cour->image) }}"></td>
                                    <td class="w-1/3 text-left py-3 px-4">{{ $cour->author }}</td>
                                    <td class="w-1/3 text-left py-3 px-4">{{ $cour->trailer }}</td>
                                    <td class="w-1/3 text-left py-3 px-4">
                                        <ul>
                                            @foreach ($cour->tags as $tags)
                                                <li>{{ $tags->name }}</li>
                                            @endforeach
                                        </ul>
                                    </td>
                                    ...
                                    <td class="w-1/3 text-left py-3 px-4">
                                        ...
                                        {{-- Delete button --}}
                                        <form action="{{ route('admin.cours.destroy', $cour->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
